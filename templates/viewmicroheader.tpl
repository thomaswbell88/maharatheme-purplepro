...
<!-- Empty class below makes small header navigation always visible -->         
...
<!-- display:block makes small header navigation always visible -->         
        <script type="text/javascript">{literal}
        addLoadEvent(function(){
            setStyle('mainnav', {'display': 'block'});
            removeElementClass('mainnav', 'js-hidden');
            connect('mainnav-container', 'onmouseenter', function(e) { appear('mainnav', {'duration': 0.25}); });
            connect('mainnav-container', 'onmouseleave', function(e) { appear('mainnav', {'duration': 0.25}); });
        });
        {/literal}
        </script>
       </div>
